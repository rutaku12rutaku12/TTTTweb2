...
export default function Component13(props){
    // [1] store에 저장된 상태를 가져오기 , useSelector( (state) => state.store등록된상태명 )
    ...
    const loginHandle = ( ) => {     // 로그인 axios 했다 가정하에 
        dispatch( login() ); // dispath 이용한 login 액션을 요청한다. dispatch( login리듀서 )
    }
    // [5] 로그아웃 처리
    const logoutHandle = ( ) =>{     // 로그아웃 axios 했다 가정하에
        dispatch( logout() ); // dispath 이용한 logout 액션을 요청한다. dispatch( logout리듀서 )
    }
