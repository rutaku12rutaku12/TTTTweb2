...
// [4] redux-persist 설정 , const persistConfig = { key : 'key이름' , storage 또는 localStorage }
import storage from 'redux-persist/lib/storage'; // localStorage 사용
import storageSession from 'redux-persist/lib/storage/session' // session 사용

// 로컬스토리지에 'user' 라는 이름으로 상태 저장
const persistConfig = { key : 'user' , storage } 

// [5] 리듀서에 persist 설정
import { persistStore , persistReducer } from 'redux-persist'
// const persistedReducer = persistedReducer( 옵션 , 설정리듀서 );
const persistedReducer = persistReducer( persistConfig , userSlice );

// [1] 스토어 만들기
const store = configureStore({
    reducer : { 
        // [2] 내가만든 슬라이스(상태) 를 등록한다. 상태명 : 슬라이스명
